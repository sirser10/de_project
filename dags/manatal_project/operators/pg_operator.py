...
from manatal_project.hooks.pg_hook import CustomPGHook
from manatal_project.helpers.common_helpers import DateGetterHelper
...
class CustomPGOperator(BaseOperator):
    """
    Custom Postgres operator for Air flow, which allows you to create ETL inside DWH,
    initialize the logic of automatic data loading into 3 primary Postgres schemas (layers):
        - src (initial raw layer)
        - stg (initial layer of normalized data)
        - ods (initial layer with distilled business data ready for operation)\n
    Based on these layers, the operator is intended primarily to call the following methods:
        - src_processor
        - stg_processor
        - ods_processor
    To call the stg_processor and ods_processor methods, it is enough to initialize the class itself with the arguments necessary for the methods (stg_cfg or ods_cfg)
    """
